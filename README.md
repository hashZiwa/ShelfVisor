...
현재 버전은 FastAPI 기반 서버와 OpenCV 기반 책등 구분 로직으로 동작합니다.

- **FastAPI**: 이미지 업로드 API와 웹 화면 제공
- **OpenCV**: 책등 사이의 세로 경계 후보 검출
- **Pillow/NumPy**: 이미지 변환, 결과 이미지 생성, 보조 계산
...
이 서비스는 이미지 처리와 OCR 연동이 핵심이므로 Python API 서버 방식이 잘 맞습니다. FastAPI는 업로드 API, 분석 결과 JSON 응답, 자동 API 문서, 향후 비동기 작업 전환에 적합합니다.

책등 구분은 현재 OpenCV 기반 데모 로직입니다.

1. 업로드 이미지를 적당한 크기로 축소합니다.
2. 서가 이미지의 상하 여백을 조금 잘라 분석 영역을 잡습니다.
3. CLAHE와 Gaussian Blur로 조명 차이를 완화합니다.
4. Canny edge와 Hough line으로 강한 세로 경계 후보를 찾습니다.
5. Lab 색공간의 Sobel 세로 변화량으로 색이 바뀌는 책등 경계를 보완합니다.
6. 가까운 경계 후보를 묶고, 너무 좁은 조각은 병합합니다.
7. 가장자리의 빈 배경처럼 활동도가 낮은 영역은 제거합니다.
...
- 실제 서가 사진 샘플을 모아 OpenCV 파라미터 조정
- 책등이 기울어진 경우를 위한 회전 보정 또는 사각형 검출 추가
