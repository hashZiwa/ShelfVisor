...
def analyze_shelf_photo(image_bytes: bytes, include_debug: bool = False) -> dict[str, Any]:
    ...
    regions = _regions_from_roboflow(roboflow_result, display_image.size)
    ...
    if include_debug:
        result["debug"] = _build_debug_payload(display_image, spines, roboflow_result)
...
def detect_book_spines(image: Image.Image) -> list[tuple[int, int, int, int]]:
    ...
    roboflow_result = infer_book_spines(model_bytes)
    return [region["box"] for region in _regions_from_roboflow(roboflow_result, image.size)]
...
def _regions_from_roboflow(result: dict[str, Any], image_size: tuple[int, int]) -> list[dict[str, Any]]:
...
def _build_debug_payload(image: Image.Image, spines: list[Spine], roboflow_result: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "stages": [
            ...
            _debug_stage("YOLO predictions", draw_annotation(image, spines)),
        ],
